...
- [🆕 Нови Функционалности (2026)](#-нови-функционалности-2026)
...
## 🆕 Нови Функционалности (2026)

### ⚙️ Config GUI (настройки без ръчна редакция)

- Нов графичен интерфейс `config_gui.py` с табове за Input, Vehicles, Warehouse, Solver, Locations, Output и Scheduler.
- Добавени са настройки за паралелно решаване в OR-Tools:
    - `enable_parallel_solving`
    - `num_workers`
    - `parallel_first_solution_strategies`
    - `parallel_local_search_metaheuristics`
- `Запази и стартирай` в EXE режим стартира solver процеса директно.

### 🌐 HTTP JSON вход (вместо Excel)

- Поддръжка на вход от URL с `input_source = "http_json"`.
- Поддръжка на автоматична бизнес дата + `json_override_date`.
- Устойчиво парсване при проблемни payload-и и различни енкодинги.

### 📄 Изходи и файлове

- Генериране на отделни HTML карти по маршрут (`routes_output_dir`).
- CSV експорт с колона за номер на поръчка.
- Подобрено поведение при output пътища:
    - абсолютните пътища от конфигурацията се запазват;
    - относителните се резолват спрямо работната директория на EXE;
    - ако е подадена директория за файл (напр. map/csv), се добавя default име.

### 🖥️ EXE режим и стартиране

- `CVRP_Optimizer.exe --settings` отваря Settings GUI.
- Генерират се удобни batch файлове:
    - `start_cvrp.bat`
    - `Settings.bat`
- Подобрена устойчивост при onefile рестарт на PyInstaller.

### 🕒 Windows Scheduler

- Добавен UI за създаване/изтриване/проверка на Scheduled Task.
- Командите са съобразени с Windows `schtasks` и работят с EXE deployment.
